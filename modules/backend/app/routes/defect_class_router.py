...
from use_cases.defect_class_use_cases import DefectClassUseCases
from db.database import get_db

router = APIRouter(prefix="/defect_class", tags=["DefectClass"])

@router.post(
    "/add",
    response_model=DefectClassSchema,
    summary="Create defect class",
    description="Create a new defect class.",
    response_description="Defect class created.",
)
def add_defect_class(defect_class: DefectClassSchema, db_session: Session = Depends(get_db)):
    """
    Endpoint para adicionar uma nova classe de defeito.
    """
    try:
        new_defect_class = DefectClassUseCases.add_defect_class(defect_class=defect_class, db_session=db_session)
...
def remove_defect_class(defect_class_id: int, db_session: Session = Depends(get_db)):
    ...
    """
    try:
        success = DefectClassUseCases.remove_defect_class_by_id(id=defect_class_id, db_session=db_session)
        if success:
...
def update_defect_class(defect_class_id: int, defect_class_data: DefectClassUpdateSchema, db_session: Session = Depends(get_db)):
    """
    Endpoint para atualizar uma classe de defeito pelo ID.
    """
    try:
        updated_defect_class = DefectClassUseCases.update_defect_class_by_id(
            id=defect_class_id, 
...
@router.get(
    "/list",
    response_model=list[DefectClassSchema],
    summary="List defect classes",
    description="List all defect classes.",
...
def list_defect_classes(db_session: Session = Depends(get_db)):
    ...
    """
    try:
        defect_classes = DefectClassUseCases.list_defect_classes(db_session=db_session)
...
def get_defect_class(defect_class_id: int, db_session: Session = Depends(get_db)):
    ...
    """
    try:
        defect_class = DefectClassUseCases.get_defect_class_by_id(id=defect_class_id, db_session=db_session)
        if defect_class:
            return defect_class
        else:
            raise HTTPException(status_code=404, detail="Classe de defeito não encontrada")
